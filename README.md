...
- **Hardcoded package names in strings**: Import statements are updated, but string literals referencing module paths are not. Packages that use string-based module references internally (config registries, plugin systems, `importlib.import_module()` calls, `__module__` checks) may break after renaming. For example, zarr internally references `"zarr.core.codec_pipeline.BatchedCodecPipeline"` — after renaming to `zarr_old`, this string won't resolve. A `--patch-strings` mode to find-replace `"original_name."` → `"new_name."` in all Python source files is planned but not yet implemented.
